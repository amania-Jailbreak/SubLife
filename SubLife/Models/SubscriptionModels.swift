...
enum SubscriptionCategory: String, Codable, CaseIterable, Identifiable {
  ...
  var label: String {
    ...
    case .entertainment: return "娯楽"
    case .internet: return "回線"
...
struct SubscriptionItem: Identifiable, Codable, Equatable {
  ...
  var installmentTotalMonths: Int? = nil
  var installmentStartDate: Date? = nil
  ...
  var monthlyEquivalent: Double {
    switch billingCycle {
    case .monthly, .installment: return price
  ...
  var billingDescription: String {
    ...
    case .installment:
      return "\(effectiveInstallmentTotalMonths)回払い・毎月\(effectiveBillingDayOfMonth)日"
    }
  }

  var effectiveInstallmentTotalMonths: Int {
    min(max(installmentTotalMonths ?? 3, 1), 240)
  }

  var effectiveInstallmentStartDate: Date {
    installmentStartDate ?? nextBillingDate
  }
  ...
  func occurs(on date: Date, calendar: Calendar = .current) -> Bool {
    ...
    case .installment:
      guard let monthOffset = installmentMonthOffset(for: target, calendar: calendar),
        monthOffset >= 0, monthOffset < effectiveInstallmentTotalMonths
      else { return false }
      occurrence = installmentOccurrence(forMonthOffset: monthOffset, calendar: calendar)
    }
  ...
  func nextChargeDate(from referenceDate: Date = .now, calendar: Calendar = .current) -> Date {
    ...
    case .yearly:
      ...
      return candidate
    case .installment:
      for offset in 0..<effectiveInstallmentTotalMonths {
        let candidate = installmentOccurrence(forMonthOffset: offset, calendar: calendar)
        if candidate >= base {
          return candidate
        }
      }
  ...
  func chargeAmount(inMonthContaining date: Date, calendar: Calendar = .current) -> Double {
    ...
    case .installment:
      guard let monthOffset = installmentMonthOffset(for: monthStart, calendar: calendar),
        monthOffset >= 0, monthOffset < effectiveInstallmentTotalMonths
      else { return 0 }
      return price
    }
  ...
  func remainingInstallmentMonths(from referenceDate: Date = .now, calendar: Calendar = .current) -> Int {
    guard billingCycle == .installment else { return 0 }
    let base = calendar.startOfDay(for: referenceDate)
    return (0..<effectiveInstallmentTotalMonths).reduce(0) { partial, offset in
      let occurrence = installmentOccurrence(forMonthOffset: offset, calendar: calendar)
      return partial + (occurrence >= base ? 1 : 0)
    }
  }

  func remainingInstallmentBalance(
    ...
    guard billingCycle == .installment else { return 0 }
    let remaining = Double(remainingInstallmentMonths(from: referenceDate, calendar: calendar))
    return amountInJPY(price * remaining, usdToJpy: usdToJpy, eurToJpy: eurToJpy)
  }
  ...
  private func installmentOccurrence(forMonthOffset offset: Int, calendar: Calendar) -> Date {
    let start = calendar.startOfDay(for: effectiveInstallmentStartDate)
    guard let targetMonth = calendar.date(byAdding: .month, value: offset, to: start) else { return start }
    var comps = calendar.dateComponents([.year, .month], from: targetMonth)
    let daysInTargetMonth = daysInMonth(of: targetMonth, calendar: calendar)
    comps.day = min(effectiveBillingDayOfMonth, daysInTargetMonth)
    return calendar.date(from: comps) ?? targetMonth
  }

  private func installmentMonthOffset(for date: Date, calendar: Calendar) -> Int? {
    let start = calendar.startOfDay(for: effectiveInstallmentStartDate)
    let startMonth = calendar.date(from: calendar.dateComponents([.year, .month], from: start)) ?? start
    let targetMonth = calendar.date(from: calendar.dateComponents([.year, .month], from: date)) ?? date
    return calendar.dateComponents([.month], from: startMonth, to: targetMonth).month
  }
