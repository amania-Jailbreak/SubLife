...
struct SettingsTabView: View {
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 14) {
                        ...
                        settingsCard(title: "テーマカラー") {
                            ScrollView(.horizontal, showsIndicators: false) {
                                HStack(spacing: 12) {
                                    ForEach(AppThemeOption.all) { option in
                                        Button {
                                            themeColorId = option.id
                                        } label: {
                                            VStack(spacing: 6) {
                                                Circle()
                                                    .fill(option.color)
                                                    ...
                                                    .overlay(
                                                        Circle()
                                                            .stroke(.white.opacity(themeColorId == option.id ? 0.95 : 0), lineWidth: 2)
                                                    )
                                                    .overlay(
                                                        Circle()
                        ...
                        settingsCard(title: "背景スタイル") {
                            ScrollView(.horizontal, showsIndicators: false) {
                                HStack(spacing: 12) {
                                    ForEach(AppBackgroundOption.all) { option in
                                        Button {
                                            backgroundThemeId = option.id
                                        } label: {
                                            VStack(spacing: 6) {
                                                RoundedRectangle(cornerRadius: 12, style: .continuous)
                                                    ...
                                                    .overlay(
                                                        RoundedRectangle(cornerRadius: 12, style: .continuous)
                                                            .stroke(option.glowColor.opacity(0.9), lineWidth: backgroundThemeId == option.id ? 2 : 0)
                                                    )
                                                Text(option.name)
                                                    .font(.caption2)
                                                    .foregroundStyle(.secondary)
                        ...
                        settingsCard(title: "データ移行") {
                            VStack(alignment: .leading, spacing: 10) {
                                Button {
                                    isPresentingExportWizard = true
                                    wizardStep = .generateKey
                                } label: {
                                    Label("エクスポート連携ウィザード", systemImage: "wand.and.sparkles")
                                        .frame(maxWidth: .infinity)
                                }
                                .buttonStyle(.borderedProminent)
                                Text("インポートは Discord の `/export` 返信リンク（`sublife://export?data=...`）経由で行います。")
                                    .font(.footnote)
                                    .foregroundStyle(.secondary)
                            }
                        }

                        settingsCard(title: "保存") {
    ...
    private func rateInputRow(
        ...
        HStack(spacing: 10) {
            ...
            TextField(placeholder, value: value, format: .number.precision(.fractionLength(0...2)))
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .stroke(focusedRateField == focus ? Color.accentColor.opacity(0.9) : .white.opacity(0.16), lineWidth: 1.2)
                )
                            ...
                            Text(step.title)
                                ...
                                .background(
                                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                                        .fill(wizardStep.rawValue >= step.rawValue ? Color.accentColor.opacity(0.27) : .white.opacity(0.08))
                                )
                        }
                    }
    ...
    private var wizardStepGenerateKey: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let generatedAt = exportKeyGeneratedAt {
                Text("生成時刻: \(generatedAt.formatted(date: .abbreviated, time: .standard))")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
    ...
    private func wizardPrimaryButton(title: String, icon: String, action: @escaping () -> Void) -> some View {
    ...
    private func wizardSecondaryButton(title: String, icon: String, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            HStack(spacing: 8) {
                Image(systemName: icon)
                Text(title)
